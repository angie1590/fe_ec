...
- [ ] T004 [P] Identify affected schemas, packaged assets, and runtime
  constraints for the feature
- [ ] T005 Confirm the work fits within the existing project architecture and
  list the exact existing modules/directories to be modified
...
- [ ] T006 Establish schema-validation and artifact-handling boundaries for the
  affected lifecycle stages
- [ ] T007 [P] Define environment configuration and secret-loading paths
- [ ] T008 [P] Update packaged assets or runtime compatibility helpers if
  required
- [ ] T009 Create base models/entities that all stories depend on
- [ ] T010 Configure error handling and inspectable diagnostics for failed XML,
  signing, or SRI exchanges
...
- [ ] T013 [P] [US1] Regression test for XML/signature/runtime behavior in
  tests/test_[name].py
...
- [ ] T019 [US1] Preserve inspectable artifacts and non-secret diagnostics for
  user story 1 operations
...
- [ ] TXXX Security hardening and secret-handling review
- [ ] TXXX Public API or migration-note updates for consumer-facing changes
...
- Work MUST stay within the existing architecture; do not add new architectural
  layers or restructure the repository
...
- Preserve consumer-facing compatibility unless the plan explicitly documents a
  breaking change
- Preserve the current project architecture at all times
